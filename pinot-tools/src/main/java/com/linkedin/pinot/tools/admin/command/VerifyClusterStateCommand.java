...
public class VerifyClusterStateCommand extends AbstractBaseAdminCommand implements Command {
  ...
  public boolean execute() throws Exception {
    ...
    if (_timeoutSec <= 0) {
      System.err.println("Error: timeoutMs must be greater than 0");
      return false;
    }

    int isStable = clusterStateVerifier.verifyClusterState(_tableName, _timeoutSec);

    if (isStable != 0) {
      String message = "Finish scanning. The cluster is not stable.";
